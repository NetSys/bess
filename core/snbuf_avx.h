...
snb_alloc_bulk(snb_array_t snbs, int cnt, uint16_t len)
{
	...
	for (i = 0; i < (cnt & (~0x1)); i+=2) {
		...
		_mm_storeu_si128((__m128i *)&snb0->mbuf.buf_len,
				mbuf_template);
		_mm_storeu_si128((__m128i *)&snb0->mbuf.packet_type,
				rxdesc_fields);

		_mm_storeu_si128((__m128i *)&snb1->mbuf.buf_len,
				mbuf_template);
		_mm_storeu_si128((__m128i *)&snb1->mbuf.packet_type,
				rxdesc_fields);
	}
	...
	if (cnt & 0x1){
		...
		_mm_storeu_si128((__m128i *)&snb->mbuf.buf_len,
				mbuf_template);
		_mm_storeu_si128((__m128i *)&snb->mbuf.packet_type,
				rxdesc_fields);
	}
