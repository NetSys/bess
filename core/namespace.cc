...
static int ns_table_init() {
  ...
  ht.item_count = 0;
  ht.ns_elem_bhead =
      (struct cdlist_head *)malloc(ht.bucket_size * sizeof(struct cdlist_head));

  if (!ht.ns_elem_bhead) return -ENOMEM;

  for (i = 0; i < ht.bucket_size; i++) cdlist_head_init(&ht.ns_elem_bhead[i]);
...
static int ns_table_resize(int new_bsize) {
  ...
  new_bhead =
      (struct cdlist_head *)malloc(new_bsize * sizeof(struct cdlist_head));
  if (!new_bhead) return -ENOMEM;

  for (int i = 0; i < new_bsize; i++) cdlist_head_init(&new_bhead[i]);
...
static int ns_table_rebalance() {
  ...
  if (ht.bucket_size == 0) return ns_table_init();
...
int ns_is_valid_name(const char *name) {
  ...
  if (name_len == 0 || name_len >= SN_NAME_LEN) return 0;

  // characters should be looks like
  // [_a-zA-Z][_a-zA-Z0-9]*

  c = name[0];
  if ((c != '_') && !isalpha(c)) return 0;
...
int ns_name_exists(const char *name) {
  ...
  if (ht.item_count == 0 || !ns_is_valid_name(name)) return 0;
...
void *ns_lookup(ns_type_t type, const char *name) {
  ...
  if (ht.item_count == 0 || !ns_is_valid_name(name)) return NULL;

  elem = ns_lookup_elem_by_name(name);

  if (elem && (elem->type == type)) return elem->object;
...
int ns_insert(ns_type_t type, const char *name, void *object) {
  ...
  if ((ret = ns_table_rebalance()) != 0) return ret;

  if (type < 0 || type >= NS_TYPE_ALL) return -EINVAL;

  if (ht.iterator_cnt[type] || ht.iterator_cnt[NS_TYPE_ALL]) return -EINVAL;

  if (!ns_is_valid_name(name)) return -EINVAL;

  if (ns_name_exists(name)) return -EEXIST;
...
void ns_init_iterator(struct ns_iter *iter, ns_type_t type) {
  ...
  *iter = (struct ns_iter){.type = type};

  if (ht.item_count == 0) return;
  ...
  if (cdlist_is_empty(ihead))
    iter->next = NULL;
  else if (iter->type == NS_TYPE_ALL)
    iter->next = container_of(ihead->next, struct ns_elem, ns_all_iter);
  else
    iter->next = container_of(ihead->next, struct ns_elem, ns_type_iter);
}
...
void *ns_next(struct ns_iter *iter) {
  ...
  if (iter->type == NS_TYPE_ALL) {
    if (cur_elem->ns_all_iter.next == (struct cdlist_item *)(ihead))
      ...
      iter->next =
          container_of(cur_elem->ns_all_iter.next, struct ns_elem, ns_all_iter);

  } else {
    if (cur_elem->ns_type_iter.next == (struct cdlist_item *)(ihead))
      iter->next = NULL;
    else
      iter->next = container_of(cur_elem->ns_type_iter.next, struct ns_elem,
                                ns_type_iter);
  }
