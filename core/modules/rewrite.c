...
static inline void do_rewrite_single(struct rewrite_priv *priv,
...
static inline void do_rewrite(struct rewrite_priv *priv,
                              struct pkt_batch *batch) {
...
static struct snobj *command_add(struct module *m, const char *cmd,
  ...
  for (i = 0; i < arg->size; i++) {
    ...
    memcpy(priv->templates[curr + i], snobj_blob_get(template), template->size);
  ...
  for (i = priv->num_templates; i < SLOTS; i++) {
    ...
    memcpy(priv->templates[i], priv->templates[j], priv->template_size[j]);
