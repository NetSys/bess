...
static struct snobj *command_set_burst(struct module *m, const char *cmd,
                                       struct snobj *arg);

static struct snobj *command_set_size(struct module *m, const char *cmd,
                                      struct snobj *arg);
...
static struct snobj *queue_init(struct module *m, struct snobj *arg) {
  ...
  if (snobj_eval_int(arg, "prefetch")) priv->prefetch = 1;
...
static void queue_deinit(struct module *m) {
  ...
  while (llring_sc_dequeue(priv->queue, (void **)&pkt) == 0) snb_free(pkt);
...
static void enqueue(struct module *m, struct pkt_batch *batch) {
  ...
  int queued =
      llring_mp_enqueue_burst(priv->queue, (void **)batch->pkts, batch->cnt);
...
static struct task_result dequeue(struct module *m, void *arg) {
  ...
  int cnt = llring_sc_dequeue_burst(priv->queue, (void **)batch.pkts, burst);
  ...
  if (priv->prefetch) {
    ...
    for (int i = 0; i < cnt; i++) total_bytes += snb_total_len(batch.pkts[i]);
  }
