...
static struct snobj *em_get_desc(const struct module *m) {
  ...
  return snobj_str_fmt("%d fields, %d rules", priv->num_fields, priv->ht.cnt);
}
...
static struct snobj *gather_key(struct em_priv *priv, struct snobj *fields,
                                hkey_t *key) {
  if (fields->size != priv->num_fields)
    return snobj_err(EINVAL, "must specify %d fields", priv->num_fields);
...
static struct snobj *command_add(struct module *m, const char *cmd,
  ...
  if ((err = gather_key(priv, fields, &key))) return err;
...
static struct snobj *command_delete(struct module *m, const char *cmd,
  ...
  if ((err = gather_key(priv, arg, &key))) return err;
