...
static void ip_lookup_process_batch(struct module *m, struct pkt_batch *batch) {
  ...
  for (i = 0; i + 3 < cnt; i += 4) {
    ...
    rte_lpm_lookupx4(priv->lpm, ip_addr, next_hops, default_gate);
  ...
  for (; i < cnt; i++) {
    ...
    ret = rte_lpm_lookup(priv->lpm, rte_be_to_cpu_32(ip->dst_addr), &next_hop);
...
struct snobj *command_add(struct module *m, const char *cmd,
                          struct snobj *arg) {
  struct ip_lookup_priv *priv = get_priv(m);
...
struct snobj *command_clear(struct module *m, const char *cmd,
                            struct snobj *arg) {
  struct ip_lookup_priv *priv = get_priv(m);
