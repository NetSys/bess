...
static inline double max_flow_pkts(const struct flowgen_priv *priv) {
  switch (priv->duration) {
    ...
    case duration_pareto:
      return scaled_pareto_variate(priv->pareto.inversed_alpha,
                                   priv->pareto.mean, priv->flow_pkts, 1.0);
    default:
      assert(0);
  }
}
...
static inline struct flow *schedule_flow(struct flowgen_priv *priv,
                                         uint64_t time_ns) {
...
static void populate_initial_flows(struct flowgen_priv *priv) {
  ...
  if (!priv->quick_rampup) return;

  if (priv->flow_pps < 1.0 || priv->flow_rate < 1.0) return;
  ...
  for (double past = step; past < past_origin; past += step) {
    ...
    if (flow_pkts > pre_consumed_pkts) {
      uint64_t jitter = 1e9 * rand_fast_real(&priv->rseed) / priv->flow_pps;
...
static struct snobj *process_arguments(struct flowgen_priv *priv,
                                       struct snobj *arg) {
  struct snobj *t;
  ...
  if (snobj_eval_int(arg, "quick_rampup")) priv->quick_rampup = 1;

  return NULL;
}

static struct snobj *init_flow_pool(struct flowgen_priv *priv) {
  /* allocate 20% more in case of temporal overflow */
  priv->allocated_flows = (int)(priv->concurrent_flows * 1.2);
  if (priv->allocated_flows < 128) priv->allocated_flows = 128;
...
static struct snobj *flowgen_init(struct module *m, struct snobj *arg) {
  ...
  if (priv->duration == duration_pareto) measure_pareto_mean(priv);
  ...
  if (priv->flow_rate > 0.0) priv->flow_gap_ns = 1e9 / priv->flow_rate;
...
static void generate_packets(struct flowgen_priv *priv,
...
static struct task_result flowgen_run_task(struct module *m, void *arg) {
  struct flowgen_priv *priv = get_priv(m);
  ...
  if (batch.cnt > 0) run_next_module(m, &batch);
...
static struct snobj *flowgen_get_dump(const struct module *m) {
...

  {
    ...
    snobj_map_set(t, "allocated_flows", snobj_int(priv->allocated_flows));
    snobj_map_set(t, "active_flows", snobj_int(priv->active_flows));
    snobj_map_set(t, "generated_flows", snobj_int(priv->generated_flows));
...

  {
    ...
    snobj_map_set(t, "total_pps", snobj_double(priv->total_pps));
    snobj_map_set(t, "flow_rate", snobj_double(priv->flow_rate));
    snobj_map_set(t, "flow_duration", snobj_double(priv->flow_duration));
...

  {
    ...
    snobj_map_set(t, "concurrent_flows", snobj_double(priv->concurrent_flows));
    snobj_map_set(t, "flow_pps", snobj_double(priv->flow_pps));
    snobj_map_set(t, "flow_pkts", snobj_double(priv->flow_pkts));
    snobj_map_set(t, "flow_gap_ns", snobj_double(priv->flow_gap_ns));
...

  {
    ...
    snobj_map_set(t, "arrival",
                  snobj_str(priv->arrival == arrival_uniform ? "uniform"
                                                             : "exponential"));
    snobj_map_set(
        t, "duration",
        snobj_str(priv->duration == duration_uniform ? "uniform" : "pareto"));
  ...
  if (priv->duration == duration_pareto) {
    ...
    snobj_map_set(t, "alpha", snobj_double(priv->pareto.alpha));
    snobj_map_set(t, "mean", snobj_double(priv->pareto.mean));
    snobj_map_set(t, "max", snobj_int(max_flow_pkts(priv)));
