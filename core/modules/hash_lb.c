...
static struct snobj *command_set_mode(struct module *m, const char *cmd,
                                      struct snobj *arg) {
...
static struct snobj *command_set_gates(struct module *m, const char *cmd,
                                       struct snobj *arg) {
  struct hlb_priv *priv = get_priv(m);
  ...
  if (snobj_type(arg) == TYPE_INT) {
    ...
    for (int i = 0; i < gates; i++) priv->gates[i] = i;
    ...
    for (int i = 0; i < gates->size; i++) {
      ...
      if (!is_valid_gate(priv->gates[i]))
        return snobj_err(EINVAL, "invalid gate %d", priv->gates[i]);
    }
...
static struct snobj *hlb_init(struct module *m, struct snobj *arg) {
  ...
  if ((t = snobj_eval(arg, "mode"))) return command_set_mode(m, NULL, t);
...
static void lb_l2(struct hlb_priv *priv, struct pkt_batch *batch,
                  gate_idx_t *ogates) {
...
static void lb_l3(struct hlb_priv *priv, struct pkt_batch *batch,
                  gate_idx_t *ogates) {
...
static void lb_l4(struct hlb_priv *priv, struct pkt_batch *batch,
                  gate_idx_t *ogates) {
...
static void hlb_process_batch(struct module *m, struct pkt_batch *batch) {
  struct hlb_priv *priv = get_priv(m);
...
static const struct mclass hlb = {
    ...
    .commands = {
        {"set_mode", command_set_mode}, {"set_gates", command_set_gates},
    }};
