...
static struct snobj *command_add(struct module *m, const char *cmd,
  ...
  for (int i = 0; i < arg->size; i++) {
    ...
    int r = l2_add_entry(&priv->l2_table, l2_addr_to_u64(addr), gate);
...
static struct snobj *command_delete(struct module *m, const char *cmd,
  ...
  for (int i = 0; i < arg->size; i++) {
    ...
    int r = l2_del_entry(&priv->l2_table, l2_addr_to_u64(addr));
...
static struct snobj *command_lookup(struct module *m, const char *cmd,
                                    struct snobj *arg) {
  struct l2_forward_priv *priv = get_priv(m);
  int i;
  ...
  for (i = 0; i < arg->size; i++) {
    ...
    gate_idx_t gate;
    int r = l2_find(&priv->l2_table, l2_addr_to_u64(addr), &gate);
...
static struct snobj *command_populate(struct module *m, const char *cmd,
                                      struct snobj *arg) {
  struct l2_forward_priv *priv = get_priv(m);
  struct snobj *t;
  char *base;
  ...
  for (int i = 0; i < cnt; i++) {
    l2_add_entry(&priv->l2_table, rte_cpu_to_be_64(base_u64 << 16),
                 i % gate_cnt);
...
static void l2_forward_process_batch(struct module *m,
  ...
  for (int i = 0; i < batch->cnt; i++) {
    ...
    l2_find(&priv->l2_table, l2_addr_to_u64(snb_head_data(snb)), &ogates[i]);
  }
