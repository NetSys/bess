...
static struct snobj *measure_init(struct module *m, struct snobj *arg) {
  ...
  if (arg) priv->warmup = snobj_eval_int(arg, "warmup");
...
static inline int get_measure_packet(struct snbuf *pkt, uint64_t *time) {
  uint8_t *avail =
      (uint8_t *)((uint8_t *)snb_head_data(pkt) + sizeof(struct ether_hdr) +
...
static void measure_process_batch(struct module *m, struct pkt_batch *batch) {
  ...
  if (priv->start_time == 0) priv->start_time = get_time();

  if (HISTO_TIME_TO_SEC(time - priv->start_time) < priv->warmup) goto skip;
...
struct snobj *command_get_summary(struct module *m, const char *cmd,
                                  struct snobj *arg) {
  struct measure_priv *priv = get_priv(m);
  ...
  snobj_map_set(r, "total_latency_ns", snobj_uint(priv->total_latency * 100));
