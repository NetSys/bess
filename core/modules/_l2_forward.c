...
struct snobj *L2Forward::Init(struct snobj *arg) {
  ...
  ret = l2_init(&this->l2_table, size, bucket);
...
void L2Forward::Deinit() { l2_deinit(&this->l2_table); }

void L2Forward::ProcessBatch(struct pkt_batch *batch) {
  gate_idx_t default_gate = ACCESS_ONCE(this->default_gate);
...
struct snobj *L2Forward::CommandAdd(struct snobj *arg) {
  ...
  for (size_t i = 0; i < arg->size; i++) {
    ...
    int r = l2_add_entry(&this->l2_table, l2_addr_to_u64(addr), gate);
...
struct snobj *L2Forward::CommandDelete(struct snobj *arg) {
  ...
  for (size_t i = 0; i < arg->size; i++) {
    ...
    int r = l2_del_entry(&this->l2_table, l2_addr_to_u64(addr));
...
struct snobj *L2Forward::CommandLookup(struct snobj *arg) {
  ...
  for (i = 0; i < arg->size; i++) {
    ...
    gate_idx_t gate;
    int r = l2_find(&this->l2_table, l2_addr_to_u64(addr), &gate);
...
struct snobj *L2Forward::CommandPopulate(struct snobj *arg) {
  ...
  for (int i = 0; i < cnt; i++) {
    l2_add_entry(&this->l2_table, rte_cpu_to_be_64(base_u64 << 16),
                 i % gate_cnt);
