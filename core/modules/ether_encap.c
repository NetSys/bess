...
static void ether_encap_process_batch(struct module *m,
  ...
  for (int i = 0; i < cnt; i++) {
    ...
    ether_src = get_attr(m, attr_r_ether_src, pkt, struct ether_addr);
    ether_dst = get_attr(m, attr_r_ether_dst, pkt, struct ether_addr);
    ...
    *ethh = (struct ether_hdr){
        .d_addr = ether_dst, .s_addr = ether_src, .ether_type = ether_type,
    };
  }
