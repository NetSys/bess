...
static struct snobj *add_field_one(struct module *m, struct snobj *field,
                                   struct field *f) {
...
static void wm_deinit(struct module *m) {
  ...
  for (int i = 0; i < priv->num_tuples; i++) ht_close(&priv->tuples[i].ht);
}
...
static void wm_process_batch(struct module *m, struct pkt_batch *batch) {
  ...
  for (int i = 0; i < priv->num_fields; i++) {
    ...
    if (attr_id < 0)
      ...
      offset = mt_offset_to_databuf_offset(mt_attr_offset(m, attr_id));
...
static struct snobj *wm_get_desc(const struct module *m) {
  ...
  return snobj_str_fmt("%d fields, %d rules", priv->num_fields, num_rules);
}
...
static struct snobj *wm_get_dump(const struct module *m) {
  ...
  for (int i = 0; i < priv->num_fields; i++) {
    ...
    if (f->attr_id < 0)
      ...
      snobj_map_set(f_obj, "name", snobj_str(m->attrs[f->attr_id].name));
...
static struct snobj *extract_key_mask(struct wm_priv *priv, struct snobj *arg,
  ...
  if (values->size != priv->num_fields)
    return snobj_err(EINVAL, "must specify %d values", priv->num_fields);

  if (!masks || snobj_type(masks) != TYPE_LIST)
    return snobj_err(EINVAL, "'masks' must be a list");

  if (masks->size != priv->num_fields)
    return snobj_err(EINVAL, "must specify %d masks", priv->num_fields);
  ...
  for (int i = 0; i < values->size; i++) {
    ...
    if (snobj_binvalue_get(v_obj, field_size, &v, force_be))
      return snobj_err(EINVAL, "idx %d: not a correct %d-byte value", i,
                       field_size);

    if (snobj_binvalue_get(m_obj, field_size, &m, force_be))
      return snobj_err(EINVAL, "idx %d: not a correct %d-byte mask", i,
                       field_size);
...
static int add_tuple(struct module *m, hkey_t *mask) {
  ...
  if (priv->num_tuples >= MAX_TUPLES) return -ENOSPC;
...
static struct snobj *command_add(struct module *m, const char *cmd,
                                 struct snobj *arg) {
  struct wm_priv *priv = get_priv(m);
...
static struct snobj *command_delete(struct module *m, const char *cmd,
                                    struct snobj *arg) {
  struct wm_priv *priv = get_priv(m);
...
static struct snobj *command_clear(struct module *m, const char *cmd,
                                   struct snobj *arg) {
  struct wm_priv *priv = get_priv(m);

  for (int i = 0; i < priv->num_tuples; i++) ht_clear(&priv->tuples[i].ht);

  return NULL;
}

static struct snobj *command_set_default_gate(struct module *m, const char *cmd,
                                              struct snobj *arg) {
  struct wm_priv *priv = get_priv(m);
