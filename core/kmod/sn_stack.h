...
static void sn_stack_init(sn_stack_t *stack) 
{
	stack->len = 0;
}

__attribute__((unused))
static inline int sn_stack_push(sn_stack_t *stack, void**objs, int len)
{
	int i;
...
__attribute__((unused))
static inline int sn_stack_pop(sn_stack_t *stack, void **objs, int len)
{
	int i;
	if (len > stack->len)
		return -1;
