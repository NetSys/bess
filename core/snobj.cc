...
static void snobj_do_free(struct snobj *m) {
  ...
  switch (m->type) {
    ...
    case TYPE_LIST:
      for (i = 0; i < m->size; i++) snobj_free(m->list.arr[i]);
...
struct snobj *snobj_list() {
  ...
  m->list.arr =
      (struct snobj **)_ALLOC(sizeof(struct snobj *) * DEF_LIST_SLOTS);
...
int snobj_list_add(struct snobj *m, struct snobj *child) {
  ...
  if (m->type != TYPE_LIST || !child) return -1;

  if (m->size == m->max_size) {
    /* too aggressive? */
    m->max_size *= 2;
    m->list.arr = (struct snobj **)_REALLOC(
        m->list.arr, m->max_size * sizeof(struct snobj *));
  }
...
static int snobj_map_index(const struct snobj *m, const char *key) {
  if (m->type != TYPE_MAP) return -1;

  for (size_t i = 0; i < m->size; i++) {
    if (strcmp(key, m->map.arr_k[i]) == 0) return i;
  }
...
int snobj_binvalue_get(struct snobj *m, uint32_t size, void *dst,
                       int force_be) {
  if (!m || size < 1) return -EINVAL;

  switch (snobj_type(m)) {
    case TYPE_BLOB:
      if (m->size != size) return -EINVAL;
      memcpy(dst, snobj_blob_get(m), m->size);
      return 0;

    case TYPE_STR:
      if (m->size != size + 1) return -EINVAL;
...
static void reserve_more(struct encode_state *s, size_t bytes) {
  ...
  if (s->offset + bytes <= s->buf_size) return;
...
struct snobj *snobj_decode_recur(struct decode_state *s) {
  ...
  switch (type) {
    ...
    case TYPE_STR:
    case TYPE_BLOB:
      m = (type == TYPE_STR) ? snobj_str_sized(s->buf + s->offset, size)
                             : snobj_blob(s->buf + s->offset, size);

      s->offset += size;
      while (s->offset % 8) s->offset++;
      break;
