...
static void do_log(int priority, const char *data, size_t len) {
  if (!initialized || FLAGS_f) {
    ...
    if (priority <= LOG_ERR) {
      ...
      if (priority <= LOG_NOTICE) color = ANSI_YELLOW;
    }

    if (color && isatty(fileno(fp)))
      fprintf(fp, "%s%.*s%s", color, (int)len, data, ANSI_RESET);
    else
  ...
  if (priority == LOG_CRIT) {
    if (!crashlog) {
      ...
      if (!(crashlog = fopen(path, "w"))) return;

      time(&curr);
      ctime_r(&curr, buf);
      *strchr(buf, '\n') = '\0';
      fprintf(crashlog,
              "Last crashlog generated at %s   "
              "----------------------------\n",
              buf);
    }
...
static void do_flush(int priority, struct logger *logger, int forced) {
  ...
  if (p != logger->buf && logger->len > 0) memmove(logger->buf, p, logger->len);
}
...
static void log_vfmt(int priority, const char *fmt, va_list ap) {
  ...
  if (initialized || priority < LOG_DEBUG) do_flush(priority, logger, 0);
}
...
void start_logger() {
  ...
  if (fd >= 0) {
    ...
    if (!FLAGS_f) {
      ...
      openlog(BESS_ID, LOG_PID | LOG_CONS | LOG_NDELAY, LOG_DAEMON);
