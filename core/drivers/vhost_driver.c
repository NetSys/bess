...
static void destroy_device (volatile struct virtio_net *dev)
{
	...
	dev->flags &= ~VIRTIO_DEV_RUNNING;
        printf("[vhost_drv]:(%" PRIu64 ") Device has been removed from socket %s\n",
	...
	while (ll_main_dev_cur != NULL) {
		...
		printf("[vhost_drv]:(%" PRIu64 ") Failed to find the dev to be destroy\n",
			dev->device_fh);
		return;
	}
...
{
        printf("[vhost_drv]:(%" PRIu64 ") Searching device '%s'\n",
                                   dev->device_fh, dev->ifname);    
#ifdef CHECK_BEFORE_ACCEPT    
        struct vhost_dev *vdev;
	struct virtio_net_data_ll *ll_main_dev_cur;
	...
	ll_main_dev_last = NULL;
        ...
        while (ll_main_dev_cur != NULL) {
                ...
                printf("[vhost_drv]:Still searching - Current device '%s'\n",
                                   vdev->name);
        ...
        if(ll_main_dev_cur == NULL){
                printf("[vhost_drv]:(%" PRIu64 ") Device '%s' can't be added - name not found\n",
                                   dev->device_fh, dev->ifname);
                return -1;
        }
        ...
        printf("[vhost_drv]:(%" PRIu64 ") Device has been added at socket %s\n",
                dev->device_fh, dev->ifname);
...
static int vhost_init_driver(struct driver *driver)
{
        ...
        if(pthread_create(&vhost_user_t, NULL, (void*)vhost_loop, NULL)) {
                printf("[vhost_drv]: Error creating thread\n");
                return -1;
        }        
...
static struct snobj *vhost_init_port(struct port *p, struct snobj *conf)
{
        ...
        printf("[vhost_drv]: Listening on socket %s for port %s\n",
                vdev->name, p->name);
...
static void vhost_deinit_port(struct port *p)
{
        ...
        if (vdev->dev != NULL) {
                printf("[vhost_drv]: Couldn't deinit port %s."
	...
	if (ret != 0)
		printf("vhost driver unregister failure.\n");
#endif        
...
static int vhost_send_pkts(struct port *p, queue_t qid, snb_array_t pkts, int cnt)
{
	struct vhost_dev *vdev = get_port_priv(p);
        uint16_t count = 0;        
        ...
        if(cnt && (vdev->dev != NULL) && (vdev->dev->flags & VIRTIO_DEV_RUNNING)){
                ...
                if (enable_retry && unlikely(cnt > rte_vring_available_entries(vdev->dev, VIRTIO_RXQ))) {
                        ...
                        for (retry = 0; retry < burst_rx_retry_num; retry++) {
                                ...
                                if (cnt <= rte_vring_available_entries(vdev->dev, VIRTIO_RXQ))
                                        break;
                        }
                }
#endif
