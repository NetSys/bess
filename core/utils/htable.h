...
#define HASH_INITVAL UINT32_MAX
...
class HTableBase {
 ...
 protected:
  ...
  int add_to_bucket(struct ht_bucket *bucket, const void *key, const void *value);
  ...
  inline void *keyidx_to_ptr(ht_keyidx_t idx) const;
  ...
  ht_hash_func_t hash_func_;
  ht_keycmp_func_t keycmp_func_;
...
inline uint32_t HTableBase::hash(const void *key) const {
  return hash_func_(key, key_size_, HASH_INITVAL);
}
...
inline void *HTableBase::keyidx_to_ptr(ht_keyidx_t idx) const {
  return (void *)((uintptr_t)entries_ + entry_size_ * idx);
}

inline ht_keyidx_t HTableBase::get_next(ht_keyidx_t curr) const {
...
inline V *HTable<K, V, C, H>::Get(const K *key) const {
  uint32_t pri = H(key, key_size_, HASH_INITVAL);
...
inline void HTable<K, V, C, H>::GetBulk(int num_keys, const K **_keys,
  ...
  for (int i = 0; i < num_keys; i++) {
    ...
    uint32_t pri = H(keys[i], key_size, HASH_INITVAL);
    pri |= (1u << 31);
    pri &= ~(1u << 30);
