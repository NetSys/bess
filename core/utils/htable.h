...
#if __AVX__
static inline ht_keyidx_t _get_keyidx_vec(const struct htable *t,
                                          uint32_t pri) {
...
#define HT_DECLARE_INLINED_FUNCS(name, key_type)                               \
                                                                               \
  static inline int name##_keycmp(const key_type *key,                         \
                                  const key_type *key_stored, size_t key_len); \
                                                                               \
  static inline uint32_t name##_hash(const key_type *key, uint32_t key_len,    \
                                     uint32_t init_val);                       \
                                                                               \
  static inline void *ht_##name##_get(const struct htable *t,                  \
                                      const void *_key) {                      \
    const key_type *key = (const key_type *)_key;                              \
    uint32_t pri = name##_hash(key, t->key_size, DEFAULT_HASH_INITVAL);        \
    pri |= (1u << 31);                                                         \
    pri &= ~(1u << 30);                                                        \
                                                                               \
    ht_keyidx_t k_idx =                                                        \
        (t->cnt >= 2048) ? _get_keyidx_vec(t, pri) : _get_keyidx(t, pri);      \
    if (k_idx == INVALID_KEYIDX) return NULL;                                  \
                                                                               \
    key_type *key_stored =                                                     \
        (key_type *)((uintptr_t)t->entries + t->entry_size * k_idx);           \
                                                                               \
  ...
  static inline void ht_##name##_get_bulk(const struct htable *t,              \
    ...
    for (int i = 0; i < num_keys; i++) {                                       \
      ...
      struct ht_bucket *sec_bucket;                                            \
                                                                               \
      uint32_t pri = name##_hash(keys[i], key_size, DEFAULT_HASH_INITVAL);     \
      pri |= (1u << 31);                                                       \
      pri &= ~(1u << 30);                                                      \
      ...
      __m256i v_hv =                                                           \
          _mm256_permute2f128_si256(v_pri_bucket, v_sec_bucket, 0x20);         \
      ...
      key_type *key_stored =                                                   \
          (key_type *)((uintptr_t)entries + entry_size * k_idx);               \
